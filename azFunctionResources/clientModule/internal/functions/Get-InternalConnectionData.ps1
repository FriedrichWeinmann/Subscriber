...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'Subscriber.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'Subscriber.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'Subscriber.Client.ProtectedToken'
