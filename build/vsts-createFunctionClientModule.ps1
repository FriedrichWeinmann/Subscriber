...
$parentModule = 'Subscriber'
if (-not $ModuleName) { $ModuleName = 'Subscriber.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Subscriber'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Subscriber.psd1"
$prereqHash = @{
